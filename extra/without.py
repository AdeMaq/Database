...
def delete_transaction(conn, transaction_id):
    ...
    conn.commit()
def create_delivery(conn, order_id, status, delivery_person, customer_address):
    cursor = conn.cursor()
    query = "INSERT INTO Delivery (orderID, deliveryDate, status, deliveryPerson, customerAddress) VALUES (%s, NOW(), %s, %s, %s)"
    cursor.execute(query, (order_id, status, delivery_person, customer_address))
    conn.commit()
...
# Array of available delivery persons
delivery_persons = [
    {"name": "John Doe", "contact": "555-1234"},
    {"name": "Jane Smith", "contact": "555-5678"},
    {"name": "Mark Johnson", "contact": "555-9876"},
]

def place_order(conn, customer_id, items, payment_method, customer_address):
    ...
    selected_delivery_person = random.choice(delivery_persons)
    
    create_delivery(conn, order_id, 'pending', selected_delivery_person['name'], customer_address)
    
    print(f"Order placed successfully! Delivery will be handled by {selected_delivery_person['name']}.")
...
def main_menu(conn):
    while True:
        ...
        if choice == '1':
            ...
            place_order(conn, customer_id, items, payment_method, customer_address)
        elif choice == '4':
